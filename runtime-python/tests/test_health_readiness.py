...
@pytest.mark.asyncio
async def test_livez_is_dependency_independent(monkeypatch):
...
@pytest.mark.asyncio
async def test_readyz_requires_runtime_components(monkeypatch):
    monkeypatch.setattr(runtime_main, "registry", None)
    monkeypatch.setattr(runtime_main, "engine", None)
...
@pytest.mark.asyncio
async def test_readyz_rejects_draining_worker(monkeypatch):
    manager = _Manager()
...
@pytest.mark.asyncio
async def test_readyz_accepts_initialized_runtime(monkeypatch):
    monkeypatch.setattr(runtime_main, "registry", object())
    monkeypatch.setattr(runtime_main, "engine", object())
    monkeypatch.setattr(runtime_main, "knowledge_scope_client", object())
