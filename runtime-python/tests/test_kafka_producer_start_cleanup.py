"""Event delivery: a failed Kafka producer startup must release the *local* producer.
...
_MODULE_NAME = "agentmesh_event_delivery_result_transport_isolated"
...
def _transport(tmp_path: Path) -> DurableKafkaResultTransport:
    return DurableKafkaResultTransport(
        ...
        topic="event-delivery.qa.results",
        client_id="event-delivery-producer-cleanup-test",
...
async def test_failed_start_keeps_outbox_and_retry_publishes_once(
    ...
    try:
        ...
        assert messages[0][0] == "event-delivery.qa.results"
