...
@pytest.mark.asyncio
async def test_concurrent_project_and_global_retrieval_isolation():
...
@pytest.mark.asyncio
async def test_project_index_http_does_not_write_runtime_memory(monkeypatch):
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=main.app), base_url="http://test") as client:
        response = await client.post(
            ...
            data={"userId": "7", "knowledgeBaseId": "11", "knowledgeFileId": "91",
                  "projectId": "1", "originalName": "project-evidence.txt", "extension": "txt"},
            files={"file": ("project-evidence.txt", b"AgentMesh private project evidence", "text/plain")},
        )
