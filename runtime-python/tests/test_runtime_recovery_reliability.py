"""Event Delivery Business ACK Reliability regressions for business ACK and attempt-level fencing."""
...
def _envelope(fence: int, token: str = "lease-1") -> DurableExecutionEnvelope:
    return DurableExecutionEnvelope(
        ...
        request=RuntimeRequest(user_id=1, request_id="event-delivery-recovery", task="hello", agents=[]),
    )
...
def _response() -> RuntimeResponse:
    return RuntimeResponse(
        request_id="event-delivery-recovery", status="COMPLETED", answer="[Mock Answer]",
        scheduler="greedy",
