...
class FakeModel:
    ...
    async def generate(self, request):
        ...
        return ModelResponse(
            content=json.dumps(
                {
                    "summary": "用户要求 AgentMesh 本地 Desktop 默认内置，并继续 Conversation Reliability 验收。",
                    "facts": ["AgentMesh 项目目录位于 E 盘"],
                    "decisions": ["Desktop 默认 Embedded，不单独启动 9583"],
                    "open_tasks": ["继续 Conversation Reliability 人工验收"],
                    "entities": ["AgentMesh", "Desktop"],
                    "keywords": ["desktop", "embedded", "conversation"],
                    "importance": 0.9,
                },
                ensure_ascii=False,
            ),
