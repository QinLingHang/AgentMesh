"""Agentless knowledge answering through the existing governed retrieval stack.
...
async def execute_knowledge_answer(
    engine,
    req: RuntimeRequest,
    *,
    ...
    def event(title: str, status: str, detail: str = "") -> None:
        entry = TraceEvent(
            kind="knowledge_answer", title=title, status=status, detail=detail,
    ...
    async def result(answer: str, *, citations=None, cost: float = 0.0) -> RuntimeResponse:
        # Preserve the same conversation-memory semantics as the main Runtime completion. Memory
