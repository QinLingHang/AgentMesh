"""Bounded model assistance for P23 task understanding.
...
def should_use_semantic_model(req, baseline) -> bool:
    from app.semantics.intent_understanding import needs_semantic_model
    return settings.p23_semantic_model_enabled and needs_semantic_model(req, baseline)
...
async def describe_with_model(engine, req) -> tuple[SemanticIntentDescriptor, dict[str, object]] | None:
    ...
    agent = AgentProfile(
        id=0,
        name="P23IntentInterpreter",
        endpoint="internal://p23-intent",
        protocol="internal",
    ...
    try:
        response = await asyncio.wait_for(
            ...
            timeout=max(0.2, float(settings.p23_semantic_model_timeout_seconds)),
        )
