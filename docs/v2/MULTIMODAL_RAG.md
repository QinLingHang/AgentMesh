...
V2 accepts the existing text-first knowledge formats plus:
...
The searchable representation is semantic text generated from this structure. This intentionally reuses the existing embedding/vector infrastructure rather than introducing a second vector database in V2.
...
The first V2 implementation normally emits `image` for standalone images and `page` for rendered PDF pages; `chart` and `table` remain valid evidence contracts for future specialized extractors.
...
V2 extends the existing provenance and citation projection with:
