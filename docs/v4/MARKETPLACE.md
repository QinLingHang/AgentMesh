# AgentMesh V4 Marketplace / Registry
...
- name
- transport（V4 为 `streamable_http`）
- endpoint
- connectTimeoutMs
- callTimeoutMs
...
V4 Plugin 采用 Registry 语义，支持 metadata、capabilities、config schema 和 permission 声明，不直接执行任意上传代码。
