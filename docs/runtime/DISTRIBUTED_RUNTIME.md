# Distributed Runtime + Resilience + Production Readiness
## 1. Goal

The distributed runtime turns the existing synchronous Go → Python execution call into an optional durable delivery plane without changing existing task semantics. `direct` remains available for compatibility; `durable` is the default UI delivery mode.
...
Everything else is fail-closed. In particular, timeout/reset/5xx after dispatch may mean the worker accepted a side-effecting execution even when Go did not receive the acknowledgment. The runtime prefers an explicit failed/ambiguous task over duplicate execution.
...
Copy `.env.production.example` to `.env.production`, replace all `CHANGE_ME` values, then validate/start with Docker Compose using the production deployment configuration.
