# Evaluation + Reliability + Cost/Latency Governance
The evaluation layer turns AgentMesh runtime output into a measurable execution product instead of a black-box answer.
...
Latency is measured end-to-end and reported post-run; The runtime does not kill an already-running task solely because wall-clock latency crossed the budget. Tool/model-specific timeouts remain the execution-level protection.
