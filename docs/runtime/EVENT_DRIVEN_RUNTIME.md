# Event-Driven Runtime & Kafka Reliability

## Goal

AgentMesh adds a Kafka event plane without replacing the existing HTTP command plane or the MySQL-backed Durable Runtime scheduler. The first production event is `runtime.execution.result`, which replaces the fragile in-memory-only HTTP result callback when `RUNTIME_RESULT_TRANSPORT=kafka`.
...
The event plane deliberately uses **at-least-once delivery + idempotent business handling** rather than claiming transport-level exactly-once semantics.
...
- `agentmesh.runtime.events` — Runtime lifecycle/result event stream. The current production integration consumes `runtime.execution.result` v1.
...
9. HTTP compatibility mode still passes distributed-runtime compatibility tests.
