# Enterprise Governance + Multi-tenant Control Plane

The governance plane extends the existing runtime without changing its core scope contract.
...
Existing project mutation endpoints remain owner-safe unless explicitly routed through the governance service. Governed resource access is checked again at runtime before task execution.
...
The Go control plane checks quota before project task execution/resume and records token/cost/tool usage after runtime completion/suspension. Durable dispatch keeps the same governance boundary.
