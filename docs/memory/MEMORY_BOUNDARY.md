...
The Memory subsystem may persist information that comes directly from the user and is safe and useful as long-term memory, for example an explicit preference or long-lived goal.
...
The Memory boundary enforces this structurally by keeping Memory as an independent service that is not injected into Knowledge ingestion, Project upload, RAG retrieval, Tool, or MCP flows.
