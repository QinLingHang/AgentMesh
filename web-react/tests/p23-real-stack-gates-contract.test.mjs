...
test("P23 case56 keeps request-local attachment IDs in the single accepted submit", () => {
...
test("P23 case72 exposes an explicit reject control bound to the approval decision API", () => {
...
test("P23 case82 UI does not equate delete governance with disabling all tools", () => {
...
test("P23 focused case56 keeps interactive delivery independent of Durable configuration", () => {
  const harness = read("e2e/v4-1-browser-e2e.mjs");
  assert.match(harness, /DURABLE_RUNTIME_ENABLED:\s*process\.env\.V4_1_E2E_P23_REAL_STACK_ONLY\s*===\s*"true"\s*\?\s*"true"\s*:\s*"false"/);
...
test("P23 Case82 fixture follows selected delete capability and proves model selection before Approval", () => {
  const harness = read("e2e/v4-1-browser-e2e.mjs");
...
test("P23 focused browser proves ENABLED to OFF restart replay reuses one Durable task", () => {
  const harness = read("e2e/v4-1-browser-e2e.mjs");
