...
test('CDP object-reference-chain diagnostic failure cannot replace the original Global assertion', async () => {
  ...
  await assert.rejects(
    () => reportFailurePreservingPrimary(original, '[Knowledge Runtime GLOBAL]', async () => {
  ...
  assert.deepEqual(logs, ['[Knowledge Runtime GLOBAL] safe failure diagnostic unavailable; original failure preserved']);
});
...
test('a broken diagnostic logger cannot replace the original project Citation failure', async () => {
  ...
  await assert.rejects(
    () => reportFailurePreservingPrimary(original, '[Knowledge Runtime PROJECT]', async () => ({ citationGuardPassed: false }), () => {
...
test('both acceptance failure paths preserve the original failure and never return raw DOM nodes by value', () => {
  ...
  assert.match(harness, /reportFailurePreservingPrimary\(error, "\[Knowledge Runtime GLOBAL\]"/);
  assert.match(harness, /reportFailurePreservingPrimary\(error, "\[Knowledge Runtime PROJECT\]"/);
