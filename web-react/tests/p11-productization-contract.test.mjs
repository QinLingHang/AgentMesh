...
test("P11/P12 browser API defaults to same-origin in development and production", () => {
...
test("P11 shell lazy-loads secondary product modules and Run Details", () => {
...
test("P11 surfaces actionable network permission and quota errors instead of raw Error strings", () => {
  const api = read("src/api.ts");
  const workspace = read("src/features/workspace/Workspace.tsx");
...
test("P11 includes a zero-dependency real-browser E2E harness", () => {
  const e2e = read("e2e/p11-browser-e2e.mjs");
  assert.match(e2e, /remote-debugging-port/);
  assert.match(e2e, /WebSocket/);
  assert.match(e2e, /P11 Browser E2E: PASS/);
...
test("P12 local direct-API fallback allows both localhost and 127.0.0.1 browser origins", () => {
...
test("P12 session restore harness uses real Go/MySQL, real Vite, both loopback hostnames, and Organization persistence", () => {
  const e2e = read("e2e/p12-dev-session-restore.mjs");
  const fixture = read("../backend-go/cmd/p12-e2e-fixture/main.go");
  ...
  assert.match(e2e, /P2_TEST_MYSQL_DSN/);
...
test("P12 final browser flow is localization/encoding independent via stable ASCII test ids", () => {
  const e2e = read("e2e/p12-dev-session-restore.mjs");
...
test("P12 session restore uses stable auth-page selector", () => {
  const auth = read("src/features/auth/Auth.tsx");
  const e2e = read("e2e/p12-dev-session-restore.mjs");
