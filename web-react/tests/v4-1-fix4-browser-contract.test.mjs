...
test("V4.1 FIX4 exposes stable conversation and Run Details browser hooks", () => {
...
test("V4.1 FIX4 real-stack harness covers all mandatory dynamic cases", () => {
...
test("V4.1 FIX5 acceptance harness uses the real Desktop ASGI module and single-fixture Go stress", () => {
  const harness = source("e2e", "v4-1-browser-e2e.mjs");
  const script = fs.readFileSync(path.join(webRoot, "..", "scripts", "TEST_V4_1_BROWSER_E2E.ps1"), "utf8");
...
test("V4.1 FIX6 authorizes the real Desktop fixture through the Bridge's canonical JSON allowlist", () => {
  const harness = source("e2e", "v4-1-browser-e2e.mjs");
...
test("V4.1 FIX7 seeds an encrypted enabled local model service for both real browser users without weakening production URL validation", () => {
  const harness = source("e2e", "v4-1-browser-e2e.mjs");
  const fixture = fs.readFileSync(path.join(webRoot, "..", "backend-go", "cmd", "p12-e2e-fixture", "main.go"), "utf8");
...
test("V4.1 FIX8 waits for the real submit gate and arms a marker-correlated delayed provider request", () => {
  const harness = source("e2e", "v4-1-browser-e2e.mjs");
...
test("V4.1 FIX9 selects a successfully created conversation before best-effort projection refresh", () => {
  const workspace = source("src", "features", "workspace", "Workspace.tsx");
...
test("V4.1 FIX10 prevents stale conversation projections from overriding an explicit current selection", () => {
...
test("V4.1 FIX11 prevents a late auto-title result from stealing current conversation", () => {
  const workspace = source("src", "features", "workspace", "Workspace.tsx");
...
test("V4.1 FIX12 keeps late direct-stream UI writes owned by the submitting conversation", () => {
  const workspace = source("src", "features", "workspace", "Workspace.tsx");
...
test("V4.1 FIX13 keeps stream ownership build-safe and provisions the real Full Runtime Agent prerequisite", () => {
  const workspace = source("src", "features", "workspace", "Workspace.tsx");
  const harness = source("e2e", "v4-1-browser-e2e.mjs");
...
test("V4.1 FIX16 models conversation history loading explicitly and waits for authoritative rows", () => {
  const app = source("src", "App.tsx");
  const workspace = source("src", "features", "workspace", "Workspace.tsx");
  const history = source("src", "features", "workspace", "MessageHistory.tsx");
  const harness = source("e2e", "v4-1-browser-e2e.mjs");
  ...
  assert.doesNotMatch(harness, /assert\.ok\(reloadedAText\.includes\(`FIX4_REPLY_\$\{aMarker\}`\)/);
});


test("V4.1 FIX17 removes stale history contracts and waits for lazy Run Details tabs", () => {
  const harness = source("e2e", "v4-1-browser-e2e.mjs");
