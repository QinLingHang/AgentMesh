...
test('Failure diagnostics retain original error and expose only structural probe data', () => {
  assert.match(browser, /reportFailurePreservingPrimary\(error, "\[Knowledge Runtime GLOBAL\]"/);
  assert.match(browser, /reportFailurePreservingPrimary\(error, "\[Knowledge Runtime PROJECT\]"/);
