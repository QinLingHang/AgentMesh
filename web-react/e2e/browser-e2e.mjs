...
const routes = new Map([
  ["GET /api/me", { id: 7, email: "browser-qa@example.com", displayName: "Browser QA", status: "ACTIVE" }],
  ...
  ["GET /api/organizations", [{ id: 11, ownerId: 7, name: "Browser QA Org", createdAt: now, updatedAt: now }]],
]);

const governance = {
  role: "OWNER",
  members: [
    { projectId: 1, userId: 7, email: "browser-qa@example.com", displayName: "Browser QA", role: "OWNER", createdAt: now, updatedAt: now },
  ],
  quota: {
    projectId: 1,
...
function createServer() {
  return http.createServer((req, res) => {
    ...
    if (requestUrl.pathname === "/__e2e__/rate-limit") {
...
async function run() {
  ...
  const profile = fs.mkdtempSync(path.join(process.cwd(), ".browser-qa-"));
  ...
  try {
    ...
    await waitFor(
      cdp,
      `document.querySelector('.app-shell') && document.querySelector('.user-card strong')?.textContent?.trim() === "Browser QA" && !document.querySelector('.auth-shell')`,
      "authenticated shell",
    );
    ...
    await fetch(`${appUrl}/__e2e__/rate-limit`);
    ...
    if (cleanupFailures.length > 0) {
      cleanupError = cleanupFailures.length === 1
        ? cleanupFailures[0]
        : new AggregateError(cleanupFailures, "browser cleanup failed");
    }
  }

  if (primaryError) {
    if (cleanupError) console.error("Browser E2E cleanup warning:", cleanupError);
    throw primaryError;
  }
