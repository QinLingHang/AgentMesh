...
async function run() {
  if (!process.env.QA_TEST_MYSQL_DSN) {
    throw new Error("QA_TEST_MYSQL_DSN is required; final browser acceptance must use an isolated real MySQL database");
  }
  ...
  const profile = fs.mkdtempSync(path.join(process.cwd(), ".session-browser-"));
  const knowledgeRoot = fs.mkdtempSync(path.join(process.cwd(), ".session-knowledge-"));
  const stamp = Date.now();
  const owner = {
    email: `browser-owner-${stamp}@example.test`,
    password: "P12OwnerPass!123",
    displayName: "Session Restore QA",
    logs: () => serverLog,
  };
  const names = {
    project: `Browser Test Project ${stamp}`,
    organization: `Browser Test Org ${stamp}`,
  };
  ...
  try {
    goServer = spawn("go", ["run", "./cmd/server"], {
      ...
      env: {
        ...
        REDIS_ADDR: process.env.BROWSER_E2E_REDIS_ADDR || "127.0.0.1:6382",
        REDIS_PASSWORD: process.env.BROWSER_E2E_REDIS_PASSWORD || "",
        REDIS_DB: process.env.BROWSER_E2E_REDIS_DB || "14",
        JWT_SECRET: "browser-jwt-secret-0123456789abcdef",
        JWT_ISSUER: "agentmesh-browser-e2e",
        JWT_ACCESS_TTL_MINUTES: "30",
        JWT_REFRESH_TTL_DAYS: "3",
        AUTH_REFRESH_COOKIE_NAME: "refresh_token",
        AUTH_COOKIE_SECURE: "false",
        RUNTIME_BASE_URL: "http://127.0.0.1:1",
        RUNTIME_INTERNAL_TOKEN: "browser-runtime-token-1234",
        ...
        GOVERNANCE_MASTER_KEY: "browser-governance-key-0123456789",
        VERIFICATION_PEPPER: "browser-verification-pepper-123",
    ...
    console.log("Real Go/MySQL Session + Organization Browser E2E: PASS");
  } catch (error) {
    primaryError = error;
  } finally {
    try { await withTimeout(closeBrowser(browserChild, cdp), 10000, "browser cleanup"); } catch (error) { cleanupError ??= error; }
    for (const [label, child] of [["Vite", vite], ["Go server", goServer]]) {
      try { await withTimeout(stopChild(child), 10000, `Release Browser ${label} cleanup`); } catch (error) { cleanupError ??= error; }
    }
    try { await withTimeout(cleanupProfile(profile), 6000, "Release Browser browser profile cleanup"); } catch (error) { cleanupError ??= error; }
    try { fs.rmSync(knowledgeRoot, { recursive: true, force: true }); } catch (error) { cleanupError ??= error; }
    try { await withTimeout(runFixture("cleanup", ["--database", fixture.database]), 35000, "browser fixture cleanup"); } catch (error) { cleanupError ??= error; }
  }

  if (primaryError) {
    if (serverLog) console.error(`real Go server log:\n${serverLog}`);
    if (cleanupError) console.error(`browser cleanup warning: ${cleanupError.stack ?? cleanupError}`);
    throw primaryError;
  }
