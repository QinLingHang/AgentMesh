...
export function Tasks({
  ...
  const [
    ...
    useState<RuntimeReliabilitySnapshot | null>(
      null,
    );
  ...
  const [
    ...
    useState<RuntimeTopologySnapshot | null>(
      null,
    );
  ...
  const cancelRunningTask =
    async (
      ...
      try {
        ...
        setFeedback({
          type: "success",
          message: `任务 #${task.id} 已请求取消。`,
        });
      } catch {
        setFeedback({
          type: "error",
          message:
            "取消失败，任务可能已经完成或进入不可撤销阶段。",
        });
      } finally {
        setCancelingId(
          null,
  ...
  return (
    <div className="page calm-page tasks-calm-page">
      ...
      {feedback && (
        <div
          className={`calm-feedback ${feedback.type}`}
          role={
            feedback.type ===
            "error"
              ? "alert"
      ...
      {deleteState && (
        ...
        >
          <div
            className="task-delete-dialog calm-dialog"
            role="dialog"
            aria-modal="true"
