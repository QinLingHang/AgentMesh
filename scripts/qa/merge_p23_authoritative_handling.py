...
The P23 router is binary, but final Handling has split ownership. Python preflight
...
def main() -> int:
    ...
    parser.add_argument("p23_predictions", type=Path)
    parser.add_argument("go_authoritative_report", type=Path)
    parser.add_argument("output", type=Path)
    args = parser.parse_args()

    cases = read_jsonl(args.dataset)
    python_rows = {row["caseId"]: row for row in read_jsonl(args.p23_predictions)}
    ...
    report = {
        "schemaVersion": "p23.handling.combined.v1",
        "ownership": {
