#!/usr/bin/env python3
"""Build deterministic Execution Routing evaluation fixtures from the reviewed merge file.
...
def base_fixture(row: dict, number: int) -> dict:
    ...
    fixture = {
        "schemaVersion": "execution-routing.eval.fixture.v1",
        "fixtureId": f"execution-route-eval-{number:03d}",
        "caseId": row["caseId"],
...
def main() -> int:
    ...
    for number, source in enumerate(rows, 1):
        ...
        fixture_ref = f"execution_routing_eval_fixtures_v1.json#{fixture['fixtureId']}"
    ...
    args.fixtures.write_text(json.dumps({
        "schemaVersion": "execution-routing.eval.fixture.catalog.v1",
