...
$KafkaName = "agentmesh-event-delivery-task-kafka-$Suffix"
$MySQLName = "agentmesh-event-delivery-task-mysql-$Suffix"
$RedisName = "agentmesh-event-delivery-task-redis-$Suffix"
$Database = "agentmesh_event_delivery_task_$Suffix"
$Topic = "event-delivery.task.$Suffix.events"
$DLQTopic = "event-delivery.task.$Suffix.dlq"
$Group = "event-delivery-task-$Suffix-group"
$KafkaPort = 39093
$MySQLPort = 43311
$RedisPort = 46383
$GoPort = 18087
...
$Email = "event-delivery-task-$Suffix@example.test"
$Password = "EVENT-DELIVERY-QA-$([guid]::NewGuid().ToString('N'))!"
$TempRoot = Join-Path $env:TEMP "agentmesh-event-delivery-task-$Suffix"
...
$GoBinary = Join-Path $TempRoot "event-delivery-qa-server.exe"
...
$EvidencePath = Join-Path $EvidenceDir "KAFKA_FINAL_TASK_OUTAGE_$Stamp-$Suffix.json"
$GoProcess = $null
$RuntimeProcess = $null
...
try {
    ...
    docker cp $TempSchema "${MySQLName}:/tmp/event-delivery-schema.sql" | Out-Null
    if($LASTEXITCODE -ne 0){throw "QA schema copy failed"}
    docker exec -e "MYSQL_PWD=$MySQLPassword" $MySQLName mysql -uroot -e "source /tmp/event-delivery-schema.sql"
    ...
    $goEnv = [ordered]@{
        ...
        KAFKA_ENABLED="true"; KAFKA_BROKERS="127.0.0.1:$KafkaPort"; KAFKA_RUNTIME_EVENTS_TOPIC=$Topic; KAFKA_RUNTIME_DLQ_TOPIC=$DLQTopic; KAFKA_RUNTIME_CONSUMER_GROUP=$Group; KAFKA_CLIENT_ID="event-delivery-task-go-$Suffix"; GOCACHE=$GoCache
    }
    ...
    $runtimeEnv = [ordered]@{
        INTERNAL_TOKEN=$InternalToken; RUNTIME_WORKER_ENABLED="true"; RUNTIME_WORKER_ID="event-delivery-task-worker-$Suffix"; RUNTIME_WORKER_ENDPOINT="http://127.0.0.1:$RuntimePort"; RUNTIME_WORKER_CAPACITY="1"
        CONTROL_PLANE_INTERNAL_BASE_URL="http://127.0.0.1:$GoPort"; RUNTIME_WORKER_HEARTBEAT_SECONDS="1"; RUNTIME_RESULT_TRANSPORT="kafka"; KAFKA_BROKERS="127.0.0.1:$KafkaPort"; KAFKA_RUNTIME_RESULT_TOPIC=$Topic; KAFKA_CLIENT_ID="event-delivery-task-python-$Suffix"; KAFKA_OUTBOX_PATH=$Outbox; KAFKA_PUBLISH_TIMEOUT_SECONDS="1"
    }
    ...
    Add-Time "local_go_python_ready" @{ workerId="event-delivery-task-worker-$Suffix" }
    ...
    $registration=Invoke-Api "POST" "/api/auth/register/verify" @{email=$Email;code=$code;password=$Password;displayName="Event Delivery Task QA"}
    ...
    $null=Invoke-Api "POST" "/api/me/model-services" @{name="Event Delivery QA Mock";provider="mock";baseUrl="https://example.invalid/v1";modelName="agentmesh-qa-mock";visionModelName="";apiKey="qa-only-not-used";enabled=$true;autoRoute=$true;isDefault=$true} $Token
    $conversation=Invoke-Api "POST" "/api/conversations" @{title="Event Delivery final outage QA"} $Token
    $ConversationId=[int64]$conversation.data.id
    $workerOnline=$false; $deadline=(Get-Date).AddSeconds(30)
    do { $topology=Invoke-Api "GET" "/api/runtime/topology" $null $Token; $workerOnline=[bool]($topology.data.workers | Where-Object {$_.workerId -eq "event-delivery-task-worker-$Suffix" -and $_.status -eq "ACTIVE"}); if($workerOnline){break}; Start-Sleep -Milliseconds 300 } while((Get-Date)-lt $deadline)
    ...
    $clientRequestId="event-delivery-task-$Suffix"
    $submit=Invoke-Api "POST" "/api/tasks/run-durable" @{clientRequestId=$clientRequestId;conversationId=$ConversationId;task="Event Delivery deterministic completion";scheduler="adaptive";planner="multi_objective";executionMode="auto";synthesisMode="auto";constraints=@{maxLatencyMs=8000;maxCost=0.15;minQuality=0.8;retryOnWorkerLoss=$false}} $Token
    ...
    Write-Output "EVENT_DELIVERY_EVIDENCE=$EvidencePath"
}

if($evidence.result -ne "PASS"){exit 1}
Write-Output "Event Delivery FINAL TASK OUTAGE: PASS"
