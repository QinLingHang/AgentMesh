#!/usr/bin/env python3
"""Run the frozen set against production Execution Routing decide_execution_routeing and score both routers."""
...
def trusted_continuation_state(fixture: dict) -> str:
    """Mirror the trusted context shape Go supplies to Execution Routing preflight.
...
def main() -> int:
    ...
    for row in cases:
        ...
        request = ExecutionRoutingRequest.model_validate({
        ...
        result = decide_execution_route(request)
        ...
        predictions.append({
            ...
            "source": "EXECUTION_ROUTING_RULE_ONLY",
        })
    args.output_dir.mkdir(parents=True, exist_ok=True)
    routing_path = args.output_dir / "execution_routing_predictions.jsonl"
    routing_path.write_text("".join(json.dumps(row, ensure_ascii=False, separators=(",", ":")) + "\n" for row in predictions), encoding="utf-8")
    report = {
        "conditions": {
            "sameDataset": args.dataset.name,
            "sameFixtures": args.fixtures.name,
            "routingModelMode": "RULE_ONLY_NO_AUTHORIZED_MODEL_CONFIG",
            "fixtureAdapter": "GO_TRUSTED_CONTEXT_V1_1",
            "handlingMetricScope": "PYTHON_PREFLIGHT_ONLY_NOT_AUTHORITATIVE_FOR_GO_TASK_OPERATIONS",
        },
        "baseline": score("Knowledge Runtime", cases, jsonl(args.baseline_predictions)),
        "current": score("SEMANTIC_ROUTER", cases, predictions),
    }
    report_path = args.output_dir / "router_eval_report.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"report": str(report_path), "baseline": report["baseline"]["metrics"]["all"],
                      "current": report["current"]["metrics"]["all"],
                      "routingSafety": report["current"]["metrics"]["safety"]}, ensure_ascii=False))
    return 0
