...
def validate_route_report(path: Path) -> tuple[bool, str]:
    try:
        ...
        all_cases = data["p23"]["metrics"]["all"]
        safety = data["p23"]["metrics"]["safety"]
...
def aggregate_gates(gates: list[dict]) -> dict[str, dict]:
    ...
    mapping = {
        ...
        "G05": ["G03_G04_G06_go_p23_targeted", "G03_G04_G05_python_p23_targeted", "G05_G08_case56_72_82_real_chrome"],
        "G06": ["G03_G04_G06_go_p23_targeted", "G08_P20_P22_full_browser_regression_OFF"],
        "G07": ["G07_live_authorization", "G05_G08_case56_72_82_real_chrome", "G08_P20_P22_full_browser_regression_OFF"],
        "G08": ["G05_G08_case56_72_82_real_chrome", "G08_P20_P22_full_browser_regression_OFF"],
        "G09": ["G09_P21_fault_injection", "G11_go_full", "G11_python_full"],
        "G10": ["G10_performance_cost"],
        "G11": ["G11_diff_check", "G11_go_full", "G11_go_server_build", "G11_go_migrate_build", "G11_python_full", "G11_react_full", "G11_react_build", "G08_react_p23_contract", "G11_driver_selftest"],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="AgentMesh P23 final acceptance driver. It never changes source, commits, pushes, or shared DB/Redis state.")
    parser.add_argument("--output-dir", default=str(ROOT / "qa-results" / "p23-final-acceptance"))
    parser.add_argument("--browser", action="store_true", help="Run isolated P23 focused real Chrome gate (56/72/82).")
    parser.add_argument("--full-browser", action="store_true", help="Also run the existing full V4.1 browser regression with P23 OFF.")
    parser.add_argument("--fault-injection", action="store_true", help="Run existing P21 outage/fault script when the Windows environment is prepared.")
    parser.add_argument("--p22-predictions", help="Frozen P22 predictions JSONL used for the 120-case P22 vs P23 route comparison.")
    ...
    gates.append(run_gate("G11_driver_selftest", [sys.executable, "-m", "unittest", "discover", "-s", str(ROOT / "scripts" / "qa"), "-p", "test_p23_final_acceptance.py"], cwd=ROOT))
    ...
    gates.append(run_gate("G03_G04_G06_go_p23_targeted", ["go", "test", "./internal/service", "-run", "TestP23|TestP5ApprovalLifecycleIntegration", "-count=1"], cwd=ROOT / "backend-go", env={"P23_AUTHORITATIVE_HANDLING_REPORT": str(authoritative_report)}))
    gates.append(run_gate("G03_G04_G05_python_p23_targeted", [sys.executable, "-m", "pytest", "-q", "--basetemp", str(targeted_temp), "tests/test_p23_binary_route_regression.py", "tests/test_p23_intent_understanding.py", "tests/test_p23_real_stack_gates.py"], cwd=ROOT / "runtime-python"))
    gates.append(run_gate("G08_react_p23_contract", ["node", "--test", "tests/p23-real-stack-gates-contract.test.mjs"], cwd=ROOT / "web-react"))
    ...
    gates.append(run_gate("G02_go_json_contract", ["go", "test", "./internal/service", "./internal/runtime", "-run", "TestP22FullRuntimeRAGPolicyJSONArrays|TestP22FullRuntimeRevocationJSONArrays|TestRuntimeDependencyCannotBecomeFastPath|TestInvalidModelOutputAndBudgetFailClosed|TestP22RuntimeValidationDiagnosticsAreRedacted", "-count=1"], cwd=ROOT / "backend-go"))
    gates.append(run_gate("G02_python_json_contract", [sys.executable, "-m", "pytest", "-q", "--basetemp", str(out / f"pytest-contract-{run_stamp}"), "tests/test_p23_intent_understanding.py"], cwd=ROOT / "runtime-python"))
    ...
    dataset = ROOT / "runtime-python" / "tests" / "fixtures" / "p23_human_eval_v1.jsonl"
    fixtures = ROOT / "runtime-python" / "tests" / "fixtures" / "p23_eval_fixtures_v1.json"
    gates.append(run_gate("G03_frozen_assets_validate", [sys.executable, str(ROOT / "scripts" / "qa" / "validate_p23_human_eval.py"), str(dataset), str(fixtures)], cwd=ROOT))
    if args.p22_predictions:
        route_out = out / "router-eval"
        gates.append(run_gate("G03_frozen_route_120", [sys.executable, str(ROOT / "scripts" / "qa" / "run_p23_frozen_eval.py"), str(dataset), str(fixtures), str(Path(args.p22_predictions).resolve()), str(route_out)], cwd=ROOT))
        apply_report_validation(gates[-1], route_out / "router_eval_report.json", validate_route_report)
    else:
        gates.append({"name": "G03_frozen_route_120", "status": "NOT_RUN", "required": True, "reason": "provide --p22-predictions so the frozen P22/P23 comparison is reproducible"})
    ...
    if args.p22_predictions and authoritative_report.exists():
        p23_predictions = out / "router-eval" / "p23_predictions.jsonl"
        combined = out / "combined_handling_report.json"
        if p23_predictions.exists():
            gates.append(run_gate("G04_authoritative_handling_merge", [sys.executable, str(ROOT / "scripts" / "qa" / "merge_p23_authoritative_handling.py"), str(dataset), str(p23_predictions), str(authoritative_report), str(combined)], cwd=ROOT))
            apply_report_validation(gates[-1], combined, validate_handling_report)
        else:
            gates.append({"name": "G04_authoritative_handling_merge", "status": "BLOCKED", "required": True, "reason": "P23 predictions were not produced"})
    else:
        gates.append({"name": "G04_authoritative_handling_merge", "status": "NOT_RUN", "required": True, "reason": "requires frozen P23 predictions and Go authoritative report"})
    ...
    if args.browser:
        focused = run_gate(
            ...
            env={"V4_1_E2E_P23_REAL_STACK_ONLY": "true", "V4_1_E2E_P23_MODE": "ENABLED"}, timeout=3000,
        )
    ...
    if args.full_browser:
        gates.append(run_gate(
            "G08_P20_P22_full_browser_regression_OFF",
            ["node", "e2e/v4-1-browser-e2e.mjs"], cwd=ROOT / "web-react",
            env={"V4_1_E2E_P23_MODE": "OFF", "V4_1_E2E_P23_REAL_STACK_ONLY": "false"}, timeout=3600,
        ))
    else:
        gates.append({"name": "G08_P20_P22_full_browser_regression_OFF", "status": "NOT_RUN", "required": True, "reason": "pass --full-browser for final closure"})
    ...
    if args.fault_injection:
        if os.name == "nt":
            gates.append(run_gate("G09_P21_fault_injection", ["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(ROOT / "scripts" / "qa" / "P21_FINAL_TASK_OUTAGE.ps1")], cwd=ROOT, env={"P23_DECISION_MODE": "OFF"}, timeout=3600))
        else:
            gates.append({"name": "G09_P21_fault_injection", "status": "BLOCKED", "required": True, "reason": "P21 outage harness is Windows PowerShell-specific"})
    else:
        gates.append({"name": "G09_P21_fault_injection", "status": "NOT_RUN", "required": True, "reason": "pass --fault-injection for final closure"})

    # G10 remains evidence-driven. Development supplies benchmark_p23_http.py;
    ...
    if performance.exists():
        ...
        gates.append({"name": "G10_performance_cost", "status": "NOT_RUN", "required": True,
                      "reason": "run benchmark_p23_http.py against matched OFF and ENABLED isolated stacks"})

    # G12 static mode default. Runtime rollback behavior is also exercised by
    # existing P23 idempotency/mode tests in Go full regression.
    env_example = ROOT / "backend-go" / ".env.example"
    env_text = env_example.read_text(encoding="utf-8") if env_example.exists() else ""
    gates.append({"name": "G12_default_off", "status": "PASS" if re.search(r"^P23_DECISION_MODE=OFF\s*$", env_text, re.MULTILINE) else "FAIL", "required": True})

    required_failures = [g for g in gates if g.get("required") and g["status"] != "PASS"]
    report = {
        "schemaVersion": "p23.final.acceptance.v1",
        "root": str(ROOT),
        "sourceFiles": {
            ...
            "qaCodeSha256": {
                ...
                for relative in (
                    ...
                    "scripts/qa/run_p23_final_acceptance.py",
                    "scripts/qa/benchmark_p23_http.py",
                    "backend-go/internal/service/p23_real_stack_gates_integration_test.go",
                    "runtime-python/tests/test_p23_real_stack_gates.py",
                )
            },
        },
        "gates": gates,
        "G01_G12": aggregate_gates(gates),
        "scopeNote": "G07 has an independent live-authorization gate; G09 remains real P21 outage evidence; G10 requires matched OFF/ENABLED real-model evidence; G12 requires default OFF plus a real Go restart from ENABLED to OFF replaying one persisted Durable request without re-execution.",
        "summary": {
    ...
    report_path = out / "p23_final_acceptance_report.json"
