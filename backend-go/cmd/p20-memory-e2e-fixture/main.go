...
func main() {
	flag.Parse()
	if flag.NArg() < 1 {
		log.Fatal("usage: p20-memory-e2e-fixture seed-history [flags]")
	}
	switch flag.Arg(0) {
	case "seed-history":
...
func adminConfig() *mysql.Config {
	...
	if dsn == "" {
		log.Fatal("P2_TEST_MYSQL_DSN is required for P20 memory E2E fixture")
	}
...
func openFixtureDatabase(name string) *sql.DB {
	if !strings.HasPrefix(name, "agentmesh_p12_browser_") {
		log.Fatal("refusing to seed outside an isolated P12/V4.1 QA database")
	}
	cfg := adminConfig()
	cfg.DBName = name
...
func seedHistory(args []string) {
	...
	markerPrefix := fs.String("marker-prefix", "P20_HISTORY", "durable message marker prefix")
	_ = fs.Parse(args)
	...
	result, err := database.ExecContext(
		...
		uid,
		fmt.Sprintf("P20 Durable History %d", time.Now().UnixNano()),
	)
	if err != nil {
		log.Fatal(err)
	}
	...
	for index := 1; index <= *count; index++ {
		...
		requestID := fmt.Sprintf("p20-memory-e2e-%03d", (index+1)/2)
