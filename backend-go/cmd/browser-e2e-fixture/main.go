...
func main() {
	flag.Parse()
	if flag.NArg() < 1 {
		log.Fatal("usage: browser-e2e-fixture <prepare|verify|cleanup|seed-v4-1-model-service> [flags]")
	}
...
func adminConfig() *mysql.Config {
	...
	if dsn == "" {
		log.Fatal("QA_TEST_MYSQL_DSN is required for the real Release Browser browser QA fixture")
	}
...
func prepare() {
	...
	name := fmt.Sprintf("agentmesh_browser_e2e_%d", time.Now().UnixNano())
	...
	memberEmail := fmt.Sprintf("browser-member-%d@example.test", time.Now().UnixNano())
...
func cleanup(args []string) {
	...
	if !strings.HasPrefix(*databaseName, "agentmesh_browser_e2e_") {
		log.Fatal("refusing to drop non-Release Browser QA database")
	}
	admin, _ := openAdmin()
	defer admin.Close()
...
func seedV41ModelService(args []string) {
	...
	if !strings.HasPrefix(*databaseName, "agentmesh_browser_e2e_") {
		log.Fatal("refusing to seed model service outside a browser/V4.1 QA database")
	}
