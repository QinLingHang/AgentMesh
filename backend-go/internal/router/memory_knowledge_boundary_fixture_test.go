...
// A distinct test package cannot reuse service's unexported QA Database test helper.
// Keep this helper test-only and use exactly the same production DDL/migrations.
func p31Database(t *testing.T) (*sql.DB, string) {
	t.Helper()
	dsn := os.Getenv("MEMORY_TEST_MYSQL_DSN")
	if dsn == "" {
		t.Skip("set MEMORY_TEST_MYSQL_DSN to run real MySQL/HTTP memory boundary acceptance")
	}
	...
	migrated.Close()
	// Existing-database upgrade: users and projects already exist before memory boundary.
...
func p31PythonKnowledgeHost(t *testing.T) string {
	...
	script, err := filepath.Abs(filepath.Join("..", "..", "..", "runtime-python", "tests", "knowledge_scope_http_fixture.py"))
	if err != nil {
		t.Fatal(err)
	}
