...
func TestConversationReliabilityConversationMemoryCapsulesAreDurableAndKeepRecentRawMessages(t *testing.T) {
	...
	uid := insert(
		...
		"conversation-reliability-capsule@example.test", "fixture", "Conversation Reliability Capsule",
	)
	conversationID := insert(
		"INSERT INTO conversations(user_id,title) VALUES(?,?)",
		uid, "Conversation Reliability memory capsule",
	)
	...
	capsule, err := repo.UpsertConversationMemoryCapsule(
		...
		model.ConversationMemoryCapsuleWrite{
			...
			Summary:         "Desktop 默认 Embedded；继续 Conversation Reliability 验收。",
...
func TestConversationReliabilityConversationMemoryOwnershipIdempotencyAndCascade(t *testing.T) {
	database, _ := p2Database(t)
	ctx := context.Background()
	repo := repository.NewMySQL(database)
	...
	uidA := insert(
		"INSERT INTO users(email,password_hash,display_name) VALUES(?,?,?)",
		"conversation-reliability-memory-a@example.test", "fixture", "Conversation Reliability Memory A",
	)
	uidB := insert(
		"INSERT INTO users(email,password_hash,display_name) VALUES(?,?,?)",
		"conversation-reliability-memory-b@example.test", "fixture", "Conversation Reliability Memory B",
	)
	conversationID := insert(
		"INSERT INTO conversations(user_id,title) VALUES(?,?)",
		uidA, "Conversation Reliability memory ownership",
	)
	startMessageID := insert(
		"INSERT INTO messages(conversation_id,role,content,status,request_id,metadata_json) VALUES(?,?,?,?,?,?)",
		conversationID, "user", "remember desktop embedded mode", "COMPLETED", "conversation-reliability-memory-own-1", "{}",
	)
	endMessageID := insert(
		"INSERT INTO messages(conversation_id,role,content,status,request_id,metadata_json) VALUES(?,?,?,?,?,?)",
		conversationID, "assistant", "desktop embedded mode is the current decision", "COMPLETED", "conversation-reliability-memory-own-1", "{}",
	)
