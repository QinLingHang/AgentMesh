...
// TaskControlOperation recognizes only unambiguous task-control commands. Generic
...
func taskCommandContainsAny(command string, values ...string) bool {
...
func normalizeTaskCommand(text string) string {
	command := strings.ToLower(strings.TrimSpace(text))
	return strings.Trim(command, " \t\r\n。！!？?，,；;")
}

// taskControlCompoundSideEffect prevents an authoritative task-control
...
func taskControlCompoundSideEffect(command string) bool {
...
// taskControlMetaDiscussion excludes requests that discuss task-control
...
func taskControlMetaDiscussion(command string) bool {
	...
	return taskCommandContainsAny(command, "概念", "步骤", "流程", "机制", "教程") &&
		taskCommandContainsAny(command, "任务", "审批", "approval", "cancel", "resume")
}

// TaskControlOperation recognizes high-confidence operations on an already-owned
...
// normal execution routing understanding path instead of guessing a Task ID or side effect.
func TaskControlOperation(text string) string {
	command := normalizeTaskCommand(text)
	if taskControlMetaDiscussion(command) || taskControlCompoundSideEffect(command) {
		return ""
	}
	switch command {
	...
	if taskCommandContainsAny(command, "审批", "approval") &&
		taskCommandContainsAny(command, "拒绝", "不同意", "不批准", "别继续写入", "reject", "deny") {
		return "APPROVAL_REJECT"
	}
	...
	priorExecution := taskCommandContainsAny(command,
	...
	if taskCommandContainsAny(command, "取消", "终止", "停止", "cancel", "stop") &&
		taskCommandContainsAny(command, "任务", "当前正在运行", "当前执行", "运行中的任务") {
		return "CANCEL_TASK"
	}
	...
	resumeVerb := taskCommandContainsAny(command,
		"恢复", "重试", "继续跑完", "继续处理", "接着处理", "继续执行", "resume", "retry")
	priorWork := taskCommandContainsAny(command,
...
// PendingTask resolves exactly one currently actionable task in the owned
// conversation. It never guesses the newest task when two candidates exist.
func (s *TaskService) PendingTask(ctx context.Context, uid int64, in RunTaskInput) (*model.Task, error) {
...
func (s *TaskService) StatusTask(ctx context.Context, uid int64, in RunTaskInput) (*model.Task, error) {
	return s.PendingTask(ctx, uid, in)
}

// ResumeCurrentTask is a safe task-state response, NOT an automatic replay.
...
func (s *TaskService) ResumeCurrentTask(ctx context.Context, uid int64, in RunTaskInput) (*RunTaskResult, error) {
	task, err := s.PendingTask(ctx, uid, in)
	if err != nil {
		return nil, err
	}
...
// RejectCurrentApproval resolves an owned, unique AUTH_REQUIRED approval
...
func (s *TaskService) RejectCurrentApproval(ctx context.Context, uid int64, in RunTaskInput) (*RunTaskResult, error) {
	if in.ConversationID == nil || s.tasks == nil {
		return nil, ErrInvalidInput
	}
	if s.projectRuntime != nil {
