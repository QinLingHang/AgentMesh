...
func P23Mode() string {
	value := strings.ToUpper(strings.TrimSpace(os.Getenv("P23_DECISION_MODE")))
	switch value {
	case "SHADOW", "ENABLED":
		return value
	}
...
func p23Fingerprint(in RunTaskInput, delivery string) (string, error) {
	if delivery == "direct" {
...
func (s *TaskService) P23ExistingSubmission(ctx context.Context, uid int64, in RunTaskInput) (*model.Task, error) {
	...
	if !ok {
		return nil, errors.New("p23 submission ledger unavailable")
	}
	...
	// The P22 modes used different canonicalization. Compare the fingerprint
	// for the ORIGINAL mode so a feature-flag change cannot change the identity.
	digest, err := p23Fingerprint(in, prior.DeliveryMode)
	if err != nil {
		return nil, err
	}
	if digest != recorded {
...
// p23ReferencesPriorContext only indicates that trusted conversation state
...
func p23ReferencesPriorContext(task string) bool {
...
func (s *TaskService) p23ContinuationState(ctx context.Context, uid int64, in RunTaskInput) (string, error) {
	if !p23ReferencesPriorContext(in.Task) {
		return "NONE", nil
	}
...
// P23Decide decides only the execution boundary. Python Runtime owns all
...
func (s *TaskService) P23Decide(ctx context.Context, uid int64, in RunTaskInput) (*P23ExecutionDecision, error) {
	return s.p23Decide(ctx, uid, in, true)
}
...
func (s *TaskService) P23DecideShadow(ctx context.Context, uid int64, in RunTaskInput) (*P23ExecutionDecision, error) {
	return s.p23Decide(ctx, uid, in, false)
}

func (s *TaskService) p23Decide(ctx context.Context, uid int64, in RunTaskInput, allowModel bool) (*P23ExecutionDecision, error) {
	...
	continuation, err := s.p23ContinuationState(ctx, uid, in)
	if err != nil {
		return nil, err
	}
	if s.runtime == nil {
	...
	proposal, err := s.runtime.UnderstandP23(ctx, runtimeclient.P23UnderstandingRequest{
		SchemaVersion: p23Version, Task: in.Task, RagMode: string(policy.Mode),
		HasAttachments:           len(in.AttachmentIDs) != 0,
		ModelReadableAttachments: s.p23ModelReadableAttachments(ctx, uid, in), ContinuationState: continuation,
	...
	decision := &P23ExecutionDecision{
		SchemaVersion: p23Version, Strategy: proposal.ExecutionRoute, Disposition: proposal.Disposition,
	...
	if err := ValidateP23Decision(*decision, *proposal, policy); err != nil {
		return nil, err
	}
	return decision, nil
}

// ValidateP23Decision is a pure, fail-closed boundary. A model never chooses
// capability IDs or overrides RAG restrictions, auth, durability or task state.
func ValidateP23Decision(decision P23ExecutionDecision, proposal runtimeclient.P23UnderstandingResponse, policy model.RagPolicy) error {
	if decision.SchemaVersion != p23Version || proposal.SchemaVersion != p23Version {
...
func P23ReplayResult(task *model.Task) *RunTaskResult {
...
func p23TaskMetadata(in RunTaskInput, metadata map[string]any) map[string]any {
	if in.P23Strategy != "" {
		metadata["p23DecisionVersion"] = p23Version
		metadata["p23Strategy"] = in.P23Strategy
		metadata["p23ReasonCodes"] = append([]string(nil), in.P23ReasonCodes...)
		metadata["p23AnalysisSource"] = in.P23AnalysisSource
		metadata["p23AnalysisLatencyMs"] = in.P23AnalysisLatencyMS
		metadata["p23PreflightModelCalls"] = in.P23PreflightModelCalls
		metadata["p23PreflightModelTokens"] = in.P23PreflightModelTokens
		if in.P23PreflightModelCostKnown {
			metadata["p23PreflightModelEstimatedCost"] = in.P23PreflightModelEstimatedCost
		}
	}
	return metadata
}

// p23DecisionTrace only includes bounded enumerated metadata. It never logs
// the user prompt, private catalog, authorization refs, or knowledge contents.
func p23DecisionTrace(in RunTaskInput, delivery string) []map[string]any {
	if in.P23Strategy == "" {
		return nil
	}
	allowed := map[string]bool{"FAST_PATH": true, "RUNTIME": true, "DIRECT_ANSWER": true, "SINGLE_CAPABILITY": true, "WORKFLOW": true}
	if !allowed[in.P23Strategy] {
		return nil
	}
	reasonCodes := make([]string, 0, 12)
	for _, reason := range in.P23ReasonCodes {
	...
	details := map[string]any{
		"decisionVersion":         p23Version,
		"strategy":                in.P23Strategy,
		"deliveryMode":            delivery,
		"reasonCodes":             reasonCodes,
		"analysisSource":          in.P23AnalysisSource,
		"analysisLatencyMs":       in.P23AnalysisLatencyMS,
		"preflightModelCalls":     in.P23PreflightModelCalls,
		"preflightModelTokens":    in.P23PreflightModelTokens,
		"preflightModelCostKnown": in.P23PreflightModelCostKnown,
	}
	if in.P23PreflightModelCostKnown {
		details["preflightModelEstimatedCost"] = in.P23PreflightModelEstimatedCost
	}
