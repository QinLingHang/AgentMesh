...
type p22PolicyJSONRepo struct {
	repository.TaskRepository
	projectID *int64
	bases     []model.KnowledgeBase
}

func (r p22PolicyJSONRepo) ResolveRuntimeKnowledgeScope(_ context.Context, uid int64, conversationID *int64) (*model.RuntimeKnowledgeScope, error) {
	return &model.RuntimeKnowledgeScope{UserID: uid, ConversationID: conversationID, ProjectID: r.projectID}, nil
}

func (r p22PolicyJSONRepo) ListKnowledgeBases(context.Context, int64) ([]model.KnowledgeBase, error) {
	if r.bases == nil {
...
func TestP22GlobalKnowledgeRequiresExplicitScope(t *testing.T) {
	projectID := int64(42)
	repo := p22PolicyJSONRepo{bases: []model.KnowledgeBase{
...
func TestP22FullRuntimeRAGPolicyJSONArrays(t *testing.T) {
	projectID := int64(42)
	for _, tc := range []struct {
		name       string
		projectID  *int64
		...
		t.Run(tc.name, func(t *testing.T) {
			svc := &TaskService{tasks: p22PolicyJSONRepo{projectID: tc.projectID}}
			...
			if tc.name == "no project desktop" {
				fixtureBytes, err := os.ReadFile("../../../runtime-python/tests/fixtures/p22_go_full_runtime_desktop.json")
				if err != nil {
					t.Fatal(err)
				}
...
func TestP22FullRuntimeRevocationJSONArrays(t *testing.T) {
