...
func routeProposal(route string) runtimeclient.P23UnderstandingResponse {
	return runtimeclient.P23UnderstandingResponse{
		SchemaVersion: p23Version, ExecutionRoute: route, Disposition: "EXECUTE", KnowledgeDependency: "NONE", ReasonCodes: []string{"CONTRACT_TEST"},
	}
}
func routeDecision(route string) P23ExecutionDecision {
	path := "FULL_RUNTIME"
	if route == "FAST_PATH" {
		path = "FAST_PATH"
	}
	return P23ExecutionDecision{SchemaVersion: p23Version, Strategy: route, Disposition: "EXECUTE", RuntimePath: path, DeliveryMode: "direct", AnalysisSource: "RULE", ReasonCodes: []string{"CONTRACT_TEST"}}
}
func TestBinaryRouteAllowsModelOnly(t *testing.T) {
	p := routeProposal("FAST_PATH")
	d := routeDecision("FAST_PATH")
	if err := ValidateP23Decision(d, p, model.RagPolicy{}); err != nil {
		t.Fatal(err)
	}
}
func TestBinaryRouteAllowsRuntimeWithoutChoosingCapabilities(t *testing.T) {
	...
	if err := ValidateP23Decision(d, p, model.RagPolicy{Mode: model.RagModeAuto}); err != nil {
		t.Fatal(err)
	}
}
func TestRuntimeDependencyCannotBecomeFastPath(t *testing.T) {
	p := routeProposal("FAST_PATH")
	p.CapabilityRequired = true
	if err := ValidateP23Decision(routeDecision("FAST_PATH"), p, model.RagPolicy{}); err == nil {
	...
	if err := ValidateP23Decision(routeDecision("FAST_PATH"), p, model.RagPolicy{Mode: model.RagModeOn}); err == nil {
...
func TestRagOffKnowledgeCannotExecute(t *testing.T) {
	...
	p.KnowledgeDependency = "REQUIRED"
	p.CapabilityRequired = true
	if err := ValidateP23Decision(routeDecision("RUNTIME"), p, model.RagPolicy{Mode: model.RagModeOff}); err == nil {
		t.Fatal("RAG OFF is a hard boundary")
	}
}
func TestModeDefaultsOffAndShadowAvailable(t *testing.T) {
	t.Setenv("P23_DECISION_MODE", "unexpected")
	if P23Mode() != "OFF" {
		t.Fatal("unsafe default")
	}
	t.Setenv("P23_DECISION_MODE", "SHADOW")
	if P23Mode() != "SHADOW" {
		t.Fatal("shadow missing")
	}
	t.Setenv("P23_DECISION_MODE", "ENABLED")
	if P23Mode() != "ENABLED" {
		t.Fatal("enabled missing")
	}
}
func TestRouteTraceDoesNotLeakTaskOrCatalog(t *testing.T) {
	events := p23DecisionTrace(RunTaskInput{P23Strategy: "RUNTIME", P23CatalogVersion: "PRIVATE_CATALOG", Task: "PRIVATE_PROMPT", P23ReasonCodes: []string{"RUNTIME_DEPENDENCY_OR_UNCERTAINTY", "private secret"}}, "durable")
	if len(events) != 1 {
...
func TestSharedPythonGoBinaryRouteFixture(t *testing.T) {
	data, err := os.ReadFile(filepath.Join("..", "..", "..", "runtime-python", "tests", "fixtures", "p23_binary_route_contract.json"))
	if err != nil {
		t.Fatal(err)
	}
	var fixture struct {
		Request  runtimeclient.P23UnderstandingRequest  `json:"request"`
		Response runtimeclient.P23UnderstandingResponse `json:"response"`
	}
	if err := json.Unmarshal(data, &fixture); err != nil {
		t.Fatal(err)
	}
	if fixture.Request.SchemaVersion != p23Version || fixture.Response.SchemaVersion != p23Version {
	...
	if err := ValidateP23Decision(d, fixture.Response, model.RagPolicy{Mode: model.RagModeAuto}); err != nil {
		t.Fatal(err)
	}
}

func TestP23ModelOnlyClarificationIsNotThirdExecutionRoute(t *testing.T) {
	p := routeProposal("FAST_PATH")
	p.Disposition = "CLARIFY"
...
func TestP23PolicyRejectionCannotLaunchRuntime(t *testing.T) {
	p := routeProposal("RUNTIME")
...
func TestP23ApprovalRejectionAndTaskStatusAreStrictCommands(t *testing.T) {
...
func TestP23DetectsPriorAssistantAnswerReferences(t *testing.T) {
	for _, text := range []string{
		"接着解释刚才的第二点。", "上一条回复最后一段能再解释一下吗？",
		"把我们刚才确认的文案再缩短一点。", "继续执行刚才的任务。",
	} {
		if !p23ReferencesPriorContext(text) {
...
func TestP23TaskOperationRecognizesFrozenAuthoritativeVariantsWithoutCapturingBusinessRequests(t *testing.T) {
	...
	for text, want := range positive {
		if got := P23TaskOperation(text); got != want {
			t.Errorf("P23TaskOperation(%q)=%q want=%q", text, got, want)
		}
	}
