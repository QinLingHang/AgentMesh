...
func TestP20DurableConversationHistoryPagesThroughEveryMessage(t *testing.T) {
	...
	uid := insert(
		...
		"p20-history@example.test", "fixture", "P20 History",
	)
	conversationID := insert(
		"INSERT INTO conversations(user_id,title) VALUES(?,?)",
		uid, "P20 durable history",
	)
	...
	for index := 1; index <= total; index++ {
		...
		insert(
			...
			conversationID, role, fmt.Sprintf("p20-message-%03d", index), "COMPLETED",
			fmt.Sprintf("p20-request-%03d", (index+1)/2), "{}",
		)
	}
	...
	for index := 1; index <= total; index++ {
		expected[fmt.Sprintf("p20-message-%03d", index)] = true
	}
