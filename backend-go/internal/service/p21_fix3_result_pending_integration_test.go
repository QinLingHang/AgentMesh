...
// Uses the ordinary P8 MySQL fixture. The test intentionally skips when no
// isolated test DSN is configured; it must never mutate the user's DB.
func TestP21ResultPendingHeartbeatSurvivesBrokerDelay(t *testing.T) {
	s, repo, uid := p8Fixture(t)
	ctx := context.Background()
	result := p8Run(t, s, uid, "result pending Kafka")
	worker := model.RuntimeWorker{WorkerID: "worker-p21", Endpoint: "http://worker.invalid", Capacity: 1}
	if err := repo.HeartbeatRuntimeWorker(ctx, worker); err != nil {
		t.Fatal(err)
	}
	job, _, err := repo.ClaimNextRuntimeJob(ctx, worker.WorkerID, "lease-p21", 2*time.Second)
	if err != nil || job == nil {
	...
	ok, err := repo.MarkRuntimeJobDispatching(ctx, job.ID, worker.WorkerID, "lease-p21")
	if err != nil || !ok {
		t.Fatalf("dispatch: %v %v", ok, err)
	}
	ok, err = repo.MarkRuntimeJobAccepted(ctx, job.ID, worker.WorkerID, "lease-p21")
	if err != nil || !ok {
		t.Fatalf("accept: %v %v", ok, err)
	}

	lease := model.RuntimeExecutionLeaseRef{
		JobID: job.ID, ExecutionID: job.ExecutionID, LeaseToken: "lease-p21",
	...
	begun, err := repo.BeginRuntimeJobCallback(ctx, job.ID, job.ExecutionID, worker.WorkerID, "lease-p21")
	if err != nil || !begun {
