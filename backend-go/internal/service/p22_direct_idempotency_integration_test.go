...
func TestP22DirectConcurrentSubmissionCreatesOneTaskAndMessage(t *testing.T) {
	database, _ := p2Database(t)
	repo := repository.NewMySQL(database)
	ctx := context.Background()
	res, err := database.ExecContext(ctx, `INSERT INTO users(email,password_hash,display_name) VALUES('p22-direct@example.test','fixture','Direct')`)
	if err != nil {
		t.Fatal(err)
	}
	...
	input := RunTaskInput{ClientRequestID: "p22-direct-concurrent-123", ConversationID: &conversationID,
