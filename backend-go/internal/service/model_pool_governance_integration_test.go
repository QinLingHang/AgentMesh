...
func TestPersonalModelPoolLifecycleAndManualRouting(t *testing.T) {
	...
	governance, err := NewGovernanceService(repo, "model-pool-governance-master-key")
	if err != nil {
		t.Fatal(err)
	}

	res, err := database.Exec("INSERT INTO users(email,password_hash,display_name) VALUES('model-pool-governance@example.test','fixture','Model Pool')")
	if err != nil {
		t.Fatal(err)
	}
...
func TestLegacyPersonalModelMigratesOnceAndDoesNotResurrectAfterDelete(t *testing.T) {
	...
	repo := repository.NewMySQL(database)
	governance, err := NewGovernanceService(repo, "model-pool-governance-legacy-master-key")
	if err != nil {
		t.Fatal(err)
	}

	res, err := database.Exec("INSERT INTO users(email,password_hash,display_name) VALUES('model-pool-governance-legacy@example.test','fixture','Legacy Model')")
	if err != nil {
		t.Fatal(err)
	}
	uid, err := res.LastInsertId()
