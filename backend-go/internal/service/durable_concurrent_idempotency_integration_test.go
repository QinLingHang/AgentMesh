...
func TestKnowledgeRuntimeDurableConcurrentSubmissionCreatesOneTaskJobAndMessage(t *testing.T) {
	database, _ := p2Database(t)
	repo := repository.NewMySQL(database)
	ctx := context.Background()
	res, err := database.ExecContext(ctx, `INSERT INTO users(email,password_hash,display_name) VALUES('knowledge-runtime-durable-race@example.test','fixture','Durable race')`)
	if err != nil {
		t.Fatal(err)
	}
	...
	const key = "knowledge-runtime-durable-race-client-key"
	const fingerprint = "012345678901234567890123456789012345678901234567890123456789abcd"
	const requestID = "knowledge-runtime-durable-race-server-request"
