...
func TestP9GovernanceRBACSecretQuotaAndAudit(t *testing.T) {
	...
	governance, err := NewGovernanceService(repo, "governance-test-master-key-32-bytes-minimum")
	if err != nil {
		t.Fatal(err)
	}
	...
	owner := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-owner@example.test','fixture','Owner')")
	developer := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-dev@example.test','fixture','Developer')")
	viewer := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-viewer@example.test','fixture','Viewer')")
	project := insert("INSERT INTO projects(user_id,name,description) VALUES(?,'Enterprise Project','fixture')", owner)

	if _, err = governance.AddMember(ctx, owner, project, "governance-dev@example.test", "DEVELOPER"); err != nil {
		t.Fatal(err)
	}
	if _, err = governance.AddMember(ctx, owner, project, "governance-viewer@example.test", "VIEWER"); err != nil {
		t.Fatal(err)
	}
	...
	secretValue := "sk-governance-super-secret-value"
...
func TestP9SharedProjectDiscoveryAndConversationAssignment(t *testing.T) {
	...
	owner := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-share-owner@example.test','fixture','Owner')")
	dev := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-share-dev@example.test','fixture','Dev')")
...
func TestP9ProjectKnowledgeSharedButOwnerGlobalKnowledgeIsNot(t *testing.T) {
	...
	owner := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-knowledge-owner@example.test','fixture','Owner')")
	member := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-knowledge-member@example.test','fixture','Member')")
	...
	globalBase := insert("INSERT INTO knowledge_bases(user_id,identity_key,name,scope,project_id,is_default) VALUES(?,'global:governance-owner','Owner Global','GLOBAL',NULL,0)", owner)
	insert("INSERT INTO project_knowledge_files(project_id,knowledge_base_id,user_id,original_name,media_type,extension,size_bytes,checksum_sha256,storage_key,status) VALUES(?,?,?,'project.txt','text/plain','txt',10,REPEAT('a',64),'governance/project.txt','READY')", project, projectBase, owner)
	insert("INSERT INTO project_knowledge_files(project_id,knowledge_base_id,user_id,original_name,media_type,extension,size_bytes,checksum_sha256,storage_key,status) VALUES(NULL,?,?, 'global.txt','text/plain','txt',10,REPEAT('b',64),'governance/global.txt','READY')", globalBase, owner)
...
func TestP9OrganizationWorkspaceAndBindingAuthorization(t *testing.T) {
	...
	owner := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-org-owner@example.test','fixture','Owner')")
	admin := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-org-admin@example.test','fixture','Admin')")
	developer := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-org-dev@example.test','fixture','Developer')")
	outsider := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-org-outsider@example.test','fixture','Outsider')")
	...
	if member, err := governance.AddOrganizationMember(ctx, owner, org.ID, "governance-org-admin@example.test", "ADMIN"); err != nil || member == nil || member.Role != "ADMIN" {
	...
	if member, err := governance.AddOrganizationMember(ctx, admin, org.ID, "governance-org-dev@example.test", "DEVELOPER"); err != nil || member == nil || member.Role != "DEVELOPER" {
		t.Fatalf("org admin could not add developer member=%+v err=%v", member, err)
	}
	if _, err := governance.AddOrganizationMember(ctx, developer, org.ID, "governance-org-outsider@example.test", "VIEWER"); err != ErrForbidden {
...
func TestP9ProjectRBACOrderingAndIDORDenial(t *testing.T) {
	...
	owner := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-rbac-owner@example.test','fixture','Owner')")
	admin := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-rbac-admin@example.test','fixture','Admin')")
	developer := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-rbac-dev@example.test','fixture','Developer')")
	viewer := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-rbac-viewer@example.test','fixture','Viewer')")
	outsider := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-rbac-outsider@example.test','fixture','Outsider')")
	project := insert("INSERT INTO projects(user_id,name,description) VALUES(?,'RBAC Project','fixture')", owner)

	if _, err := governance.AddMember(ctx, owner, project, "governance-rbac-admin@example.test", "ADMIN"); err != nil {
		t.Fatal(err)
	}
	if _, err := governance.AddMember(ctx, admin, project, "governance-rbac-dev@example.test", "DEVELOPER"); err != nil {
		t.Fatal(err)
	}
	if _, err := governance.AddMember(ctx, admin, project, "governance-rbac-viewer@example.test", "VIEWER"); err != nil {
		t.Fatal(err)
	}
...
func TestP9SharedProjectDoesNotShareUserGlobalMemory(t *testing.T) {
	...
	owner := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-memory-owner@example.test','fixture','Owner')")
	member := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-memory-member@example.test','fixture','Member')")
	project := insert("INSERT INTO projects(user_id,name,description) VALUES(?,'Memory Shared Project','fixture')", owner)
	if _, err := governance.AddMember(ctx, owner, project, "governance-memory-member@example.test", "DEVELOPER"); err != nil {
		t.Fatal(err)
	}
...
func TestP9QuotaDimensionsFailClosedAndSuppressRuntime(t *testing.T) {
	newFixture := func(t *testing.T) (*repository.MySQL, *GovernanceService, int64, int64, int64) {
		...
		owner := insert("INSERT INTO users(email,password_hash,display_name) VALUES(CONCAT('governance-quota-owner-',UUID(),'@example.test'),'fixture','Owner')")
		developer := insert("INSERT INTO users(email,password_hash,display_name) VALUES(CONCAT('governance-quota-dev-',UUID(),'@example.test'),'fixture','Developer')")
	...
	t.Run("ConcurrentTasks", func(t *testing.T) {
		...
		if _, err := repo.CreateTask(ctx, model.Task{UserID: developer, ConversationID: &convID, RequestID: "governance-concurrent-active", TaskText: "active", Scheduler: "greedy", Planner: "heuristic", ExecutionMode: "auto", SynthesisMode: "auto"}, model.TaskConstraints{MaxLatencyMS: 1000, MaxCost: 1, MinQuality: .8}); err != nil {
			t.Fatal(err)
		}
		if err := governance.CheckQuota(ctx, developer, project); err != ErrQuotaExceeded {
...
func TestP9AuditActorResourceResultAndSensitiveMetadataRedaction(t *testing.T) {
	...
	owner := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-audit-owner@example.test','fixture','Owner')")
...
func TestP9SharedProjectExecutionUsesOwnerResourcesAndRecordsRuntimeUsage(t *testing.T) {
	...
	owner := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-exec-owner@example.test','fixture','Owner')")
	developer := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-exec-dev@example.test','fixture','Developer')")
	project := insert("INSERT INTO projects(user_id,name,description) VALUES(?,'Execution Shared Project','fixture')", owner)
	if _, err := governance.AddMember(ctx, owner, project, "governance-exec-dev@example.test", "DEVELOPER"); err != nil {
		t.Fatal(err)
	}
	...
	// Configure the Project owner's provider explicitly so this governance regression
	// continues to verify owner-scoped runtime resources without relying on any
	// platform-global model credential.
	providerSecret, err := governance.CreateSecret(ctx, owner, project, "PROJECT_SHARED_MODEL_API_KEY", "MODEL_API_KEY", "sk-shared-project-fixture")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := governance.UpsertProvider(ctx, owner, project, model.ProjectModelProvider{
		Provider: "openai-compatible", BaseURL: "https://api.example.test/v1", ModelName: "shared-owner-model", SecretID: &providerSecret.ID, Enabled: true,
	}); err != nil {
		t.Fatal(err)
	}
	...
	case req := <-requestSeen:
		...
		if req.ProjectModel == nil || req.ProjectModel.ModelName != "shared-owner-model" || req.ProjectModel.APIKey != "sk-shared-project-fixture" {
...
func TestP9ViewerCannotFallThroughToSharedProjectRuntime(t *testing.T) {
	...
	owner := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-viewexec-owner@example.test','fixture','Owner')")
	viewer := insert("INSERT INTO users(email,password_hash,display_name) VALUES('governance-viewexec-viewer@example.test','fixture','Viewer')")
	project := insert("INSERT INTO projects(user_id,name,description) VALUES(?,'Viewer Protected Project','fixture')", owner)
	if _, err := governance.AddMember(ctx, owner, project, "governance-viewexec-viewer@example.test", "VIEWER"); err != nil {
		t.Fatal(err)
	}
