...
const p23ClarificationKind = "p23_clarification"

// PersistP23Clarification records the user turn and the clarification prompt as
// one idempotent, suspended logical task. It creates no Runtime job and invokes
// no Tool/Agent/Knowledge capability.
func (s *TaskService) PersistP23Clarification(
	ctx context.Context,
	uid int64,
	in RunTaskInput,
	...
	in.P23Strategy = decision.Strategy
	in.P23ReasonCodes = append([]string(nil), decision.ReasonCodes...)
	in.P23AnalysisSource = decision.AnalysisSource
	in.P23AnalysisLatencyMS = decision.AnalysisLatencyMS
	in.P23PreflightModelCalls = decision.PreflightModelCalls
	in.P23PreflightModelTokens = decision.PreflightModelTokens
	in.P23PreflightModelEstimatedCost = decision.PreflightModelEstimatedCost
	in.P23PreflightModelCostKnown = decision.PreflightModelCostKnown
	...
	task, replayed, err := s.insertDirectTask(ctx, model.Task{
		...
		PendingUserMessageMetadata: p23TaskMetadata(in, map[string]any{
			"runtimePhase": "p23_clarification", "status": "INPUT_REQUIRED",
		}),
	}, in.Constraints)
	if err != nil {
		return nil, err
	...
	continuation := &model.TaskContinuation{
		Protocol: p23ClarificationKind, AgentID: 0, Capability: "clarification",
		TaskID: "clarification", ContextID: p23Version,
		State: "INPUT_REQUIRED", Kind: p23ClarificationKind, Summary: message,
	}
	...
	if in.ConversationID != nil {
		assistant = &repository.AssistantMessageWrite{
			...
			Metadata: map[string]any{
				"taskId": task.ID, "runtimePhase": "p23_clarification",
				"status": "INPUT_REQUIRED", "p23Clarification": true,
			},
		}
	}
	trace := p23DecisionTrace(in, "direct")
...
// ResolveP23Clarification marks a consumed clarification wait as completed. It
...
func (s *TaskService) ResolveP23Clarification(ctx context.Context, uid, taskID int64) error {
	if taskID <= 0 {
		return nil
	}
	...
	if task.Status != "INPUT_REQUIRED" || task.Continuation == nil || task.Continuation.Kind != p23ClarificationKind {
		return nil
	}
...
func (s *TaskService) SupersedeP23Clarifications(ctx context.Context, uid int64, completed *model.Task) error {
	...
	for _, item := range items {
		if item.ID >= completed.ID || item.ConversationID == nil ||
			*item.ConversationID != *completed.ConversationID || item.Status != "INPUT_REQUIRED" ||
			item.Continuation == nil || item.Continuation.Kind != p23ClarificationKind {
			continue
		}
		if err := s.ResolveP23Clarification(ctx, uid, item.ID); err != nil {
			return err
		}
	}
	return nil
