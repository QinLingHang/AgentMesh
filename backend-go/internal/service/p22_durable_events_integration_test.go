...
func TestP22DurableEventReplayOwnershipAndFence(t *testing.T) {
	...
	if err := repo.HeartbeatRuntimeWorker(ctx, model.RuntimeWorker{
		WorkerID: "p22-events-worker", Endpoint: "http://runtime.invalid:9572", Capacity: 1,
	}); err != nil {
		t.Fatal(err)
	}
	job, _, err := repo.ClaimNextRuntimeJob(ctx, "p22-events-worker", "p22-events-lease", time.Second)
	if err != nil || job == nil {
