...
func TestP22DurableSubmissionIdempotency(t *testing.T) {
	database, _ := p2Database(t)
	repo := repository.NewMySQL(database)
	ctx := context.Background()
	createdUser, err := database.ExecContext(ctx, `INSERT INTO users(email,password_hash,display_name) VALUES('p22-idempotency@example.test','fixture','P22')`)
	if err != nil {
		t.Fatal(err)
	}
