...
// Reuses the existing isolated-MySQL fixture from QA Database so Evaluation validates the real
...
func TestP6ScorecardPersistsInAssistantMessageMetadata(t *testing.T) {
	...
	uid := insert("INSERT INTO users(email,password_hash,display_name) VALUES('evaluation-scorecard@example.test','fixture','Evaluation Scorecard')")
	conversationID := insert("INSERT INTO conversations(user_id,title) VALUES(?,'Evaluation persistence')", uid)
	...
	result, err := tasks.Run(ctx, uid, RunTaskInput{
		...
		Task:           "validate evaluation scorecard persistence",
		Scheduler:      "greedy",
		Planner:        "heuristic",
