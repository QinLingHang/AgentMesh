...
func p8Fixture(t *testing.T) (*DurableRuntimeService, *repository.MySQL, int64) {
	...
	result, err := database.Exec(`INSERT INTO users(email,password_hash,display_name) VALUES('p8@example.test','fixture','P8')`)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err = database.Exec(`
		...
		VALUES(?,'P8 General','internal://success','internal','["general"]','ACTIVE')
	`, uid); err != nil {
		t.Fatal(err)
	}
...
func TestP8DurableQueueLeaseFencingAndCallbackPersistence(t *testing.T) {
	s, repo, uid := p8Fixture(t)
	ctx := context.Background()
	result := p8Run(t, s, uid, "durable completion")

	job, raw, err := repo.ClaimNextRuntimeJob(ctx, "worker-p8", "lease-p8", time.Second)
	...
	if err = repo.HeartbeatRuntimeWorker(ctx, model.RuntimeWorker{
		WorkerID: "worker-p8", Endpoint: "http://runtime.invalid:9572", Capacity: 1,
	}); err != nil {
		t.Fatal(err)
	}
	job, raw, err = repo.ClaimNextRuntimeJob(ctx, "worker-p8", "lease-p8", time.Second)
	...
	ok, err := repo.MarkRuntimeJobDispatching(ctx, job.ID, "worker-p8", "lease-p8")
	if err != nil || !ok {
		t.Fatalf("dispatch fence: ok=%v err=%v", ok, err)
	}
	ok, err = repo.MarkRuntimeJobAccepted(ctx, job.ID, "worker-p8", "lease-p8")
	if err != nil || !ok {
		t.Fatalf("accept fence: ok=%v err=%v", ok, err)
	}

	callback := DurableExecutionCallback{
		WorkerID: "worker-p8", ExecutionID: job.ExecutionID, LeaseToken: "lease-p8", Status: "completed",
		Response: &runtimeclient.ExecuteResponse{
			...
			SelectedAgents: []string{"P8 General"}, Trace: []map[string]any{}, DAG: map[string]any{},
		},
	}
...
func TestP8LeaseRecoveryCapacityCancelAndCircuitBreaker(t *testing.T) {
	s, repo, uid := p8Fixture(t)
	ctx := context.Background()
	if err := repo.HeartbeatRuntimeWorker(ctx, model.RuntimeWorker{
...
func TestP8ReliabilitySnapshotNeverExposesWorkerEndpoint(t *testing.T) {
	s, repo, uid := p8Fixture(t)
	ctx := context.Background()
	_ = uid
	if err := repo.HeartbeatRuntimeWorker(ctx, model.RuntimeWorker{
