...
func TestP5ApprovalLifecycleIntegration(t *testing.T) {
	...
	uid := insert("INSERT INTO users(email,password_hash,display_name) VALUES('approval@example.test','fixture','Secure Action')")
	projectID := insert("INSERT INTO projects(user_id,name,description) VALUES(?,'Secure Action','fixture')", uid)
	conversationID := insert("INSERT INTO conversations(user_id,title) VALUES(?,'Secure Action fixture')", uid)
	...
	mcpID := insert("INSERT INTO mcp_servers(user_id,name,transport,endpoint,enabled) VALUES(?,'Secure Action MCP','streamable_http','http://fixture/mcp',1)", uid)
	...
	newApprovalTask := func(t *testing.T, requestID, protocol string, arguments map[string]any, conversationOverride ...int64) *model.Task {
		t.Helper()
		// Existing secure-action subtests intentionally share their original conversation.
		// A Execution Routing natural-language approval test can supply an isolated conversation
	...
	t.Run("ExecutionRoutingRejectApprovalReusesAuthoritativeResume", func(t *testing.T) {
		...
		approvalConversationID := insert("INSERT INTO conversations(user_id,title) VALUES(?,'Execution Routing approval fixture')", uid)
		insert("INSERT INTO project_conversations(project_id,conversation_id) VALUES(?,?)", projectID, approvalConversationID)
		created := newApprovalTask(t, "routing-reject", "http", map[string]any{"order_id": "ORDER-ROUTING"}, approvalConversationID)
		result, err := tasks.RejectCurrentApproval(ctx, uid, RunTaskInput{
		...
		if req.Task != "reject" || req.Continuation == nil || req.Continuation.ToolName != "cancel_order" {
			t.Fatal("Execution Routing rejection did not use original persisted approval")
		}
		mu.Lock()
		requestCountBeforeReplay := len(requests)
		mu.Unlock()
		if _, err := tasks.RejectCurrentApproval(ctx, uid, RunTaskInput{ConversationID: &approvalConversationID}); !errors.Is(err, ErrNotFound) {
	...
	t.Run("ExecutionRoutingRejectApprovalRefusesAmbiguousPendingActions", func(t *testing.T) {
		ambiguousConversationID := insert("INSERT INTO conversations(user_id,title) VALUES(?,'Execution Routing ambiguous approvals')", uid)
		insert("INSERT INTO project_conversations(project_id,conversation_id) VALUES(?,?)", projectID, ambiguousConversationID)
		newApprovalTask(t, "routing-ambiguous-one", "http", map[string]any{"order_id": "ORDER-1"}, ambiguousConversationID)
		newApprovalTask(t, "routing-ambiguous-two", "http", map[string]any{"order_id": "ORDER-2"}, ambiguousConversationID)
		mu.Lock()
		before := len(requests)
		mu.Unlock()
		_, err := tasks.RejectCurrentApproval(ctx, uid, RunTaskInput{ConversationID: &ambiguousConversationID})
